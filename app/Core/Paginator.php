    ...
    public function run($query, $where, $whereBindings, $formatter = null, $countQuery = null)
    {
        try {
            ...
            if ($formatter) {
                ...
                $needsAlumniGrouping = !empty($results) && isset($results[0]["urole"]) && $results[0]["urole"] === "alumni";
                ...
                if ($needsVacancyGrouping) {
                    ...
                    foreach ($results as $result) {
                        ...
                        if (!isset($grouped[$uid])) {
                            $grouped[$uid] = $result;
                            $grouped[$uid]["occupations"] = [];
                            $grouped[$uid]["socials"] = [];
                        }
    
                        if (!empty($result["osid"])) {
                            $osid = $result["osid"];
                            ...
                            if (empty($alreadyAdded)) {
                                $grouped[$uid]["occupations"][] = [
                                    "id"            => $result["osid"],
                                    "alumni_id"     => $result["osalumni_id"],
                                    "occupation_id" => $result["osoccupation_id"],
                        ...
                        if (!empty($result["sid"])) {
                            ...
                            if (empty($alreadyAdded)) {
                                $grouped[$uid]["socials"][] = [
                                    "id"          => $result["sid"],
                                    "alumni_id"   => $result["salumni_id"],
                                    "platform_id" => $result["splatform_id"],
                                    "platform"    => $result["plname"],
                                    "url"         => $result["surl"],
                                ];
                            }
                        }
